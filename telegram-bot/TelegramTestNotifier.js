...
class TelegramTestNotifier {
  constructor() {
    ...
    if (!this.token || !this.chatId) {
      console.warn('⚠️ Telegram bot credentials not found. Notifications disabled.');
      this.enabled = false;
      return;
    }
  ...
  formatMessage(testData, coverageData, options = {}) {
    ...
    if (testData.failed > 0) {
      ...
      message += `⚠️ **Status:** NO TESTS - No tests were found\n`;
    } else {
  ...
  async testConnection() {
    ...
    try {
      ...
      // Send test message
      await this.bot.sendMessage(this.chatId, '🤖 Test notification: Bot is working!');
