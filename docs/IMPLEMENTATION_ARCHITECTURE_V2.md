# 🚀 Book Catalog App - Implementation Architecture V2.0
...
- **Related**: [PRD V2.0](./PRD_Book_Catalog_V2.md), [System Architecture](./SYSTEM_ARCHITECTURE.md)

---

## 🎯 Implementation Overview

This document provides detailed implementation guidelines for Book Catalog App V2, including code examples, configuration setups, and deployment strategies for the enhanced features.

---

## 🔄 Phase 1: Supabase Migration Implementation
...
CREATE INDEX idx_books_title_author ON public.books USING GIN (to_tsvector('english', title || ' ' || author));
CREATE INDEX idx_books_category ON public.books(category);
CREATE INDEX idx_books_status ON public.books(reading_status);
...
class DatabaseService {
  ...
  // Get user books with search
  async getUserBooks(userId, searchParams = {}) {
    try {
      ...
      const sortOrder = searchParams.sort_order === 'asc' ? { ascending: true } : { ascending: false };
...
### 2.1 Enhanced Book Routes

```javascript
// backend/routes/bookRoutes.js - Enhanced with search
const express = require('express');
const router = express.Router();
const db = require('../db');
const authMiddleware = require('../middlewares/authMiddleware');
const { validateBookData, validateSearchParams } = require('../middlewares/validation');
...
router.get('/search', authMiddleware, searchLimiter, validateSearchParams, async (req, res) => {
  ...
  try {
    ...
    const result = await db.getUserBooks(userId, searchParams);
    ...
    res.json({
      ...
      performance: {
        ...
        results_count: result.books.length
      }
    });
  } catch (error) {
...
// Get all books (with basic search)
...
// Create book
router.post('/', authMiddleware, validateBookData, async (req, res) => {
  try {
    const userId = req.user.id;
    const bookData = {
    ...
    const book = await db.createBook(bookData, userId);
...
// Update book
router.put('/:id', authMiddleware, validateBookData, async (req, res) => {
  try {
    const userId = req.user.id;
    ...
    const book = await db.updateBook(bookId, bookData, userId);
...
router.delete('/:id', authMiddleware, async (req, res) => {
  try {
    ...
    await db.deleteBook(bookId, userId);
...
router.post('/bulk', authMiddleware, async (req, res) => {
  try {
    const { action, book_ids, data } = req.body;
    const userId = req.user.id;
    ...
    switch (action) {
      case 'update_status':
        for (const bookId of book_ids) {
          try {
            const book = await db.updateBook(bookId, data, userId);
            results.push({ id: bookId, success: true, data: book });
          } catch (error) {
            results.push({ id: bookId, success: false, error: error.message });
          }
        }
        break;
      ...
      case 'delete':
        for (const bookId of book_ids) {
          try {
            await db.deleteBook(bookId, userId);
            results.push({ id: bookId, success: true });
          } catch (error) {
            results.push({ id: bookId, success: false, error: error.message });
          }
        }
...
const validateSearchParams = [
  ...
  query('sort_by')
    .optional()
    .isIn(['title', 'author', 'created_at', 'updated_at', 'rating', 'publication_year'])
    .withMessage('Sort field must be: title, author, created_at, updated_at, rating, or publication_year'),
...
## 🔐 Phase 3: Forgot Password Implementation
...
const { validateForgotPassword, validateResetPassword } = require('../middlewares/validation');
...
router.post('/forgot-password', passwordResetLimiter, validateForgotPassword, async (req, res) => {
  try {
    ...
    await emailService.sendPasswordResetEmail(email, resetUrl, user.user.user_metadata?.name || 'User');
...
router.get('/reset-password/:token', async (req, res) => {
  try {
    ...
    const { data: user } = await supabaseAdmin.auth.admin.getUserById(resetTokenData.user_id);
...
class EmailService {
  ...
  async initializeTransporter() {
    ...
    try {
      ...
      console.error('❌ SMTP email service failed:', error.message);
    }
  }
  ...
  async sendPasswordResetEmail(email, resetUrl, userName = 'User') {
    const subject = 'Password Reset Request - Book Catalog';
    ...
    const html = `
      ...
      <body>
        <div class="container">
          ...
          <div class="content">
            <p>Hello ${userName},</p>
            <p>You requested a password reset for your Book Catalog account. Click the button below to reset your password:</p>
          ...
          <div class="footer">
            <p>Book Catalog App - Secure Password Reset</p>
    ...
    const text = `
      ...
      You requested a password reset for your Book Catalog account.
  ...
  async sendPasswordResetConfirmation(email, userName = 'User') {
    const subject = 'Password Reset Successful - Book Catalog';
    ...
    const html = `
      ...
      <body>
        <div class="container">
          ...
          <div class="content">
            <p>Hello ${userName},</p>
            <p>Your password has been successfully reset for your Book Catalog account.</p>
          ...
          <div class="footer">
            <p>Book Catalog App - Security Notification</p>
            <p>This is an automated email, please do not reply.</p>
          </div>
        </div>
      </body>
    ...
    const text = `
      ...
      Your password has been successfully reset for your Book Catalog account.
  ...
  async sendTestEmail(email) {
    const subject = 'Test Email - Book Catalog';
    const html = '<h1>Test Email</h1><p>Email service is working correctly!</p>';
...
global.testUtils = {
  ...
  async createTestBook(userId, bookData = {}) {
    const defaultBook = {
      title: 'Test Book',
      author: 'Test Author',
      category: 'Fiction',
    ...
    return await db.createBook({ ...defaultBook, ...bookData }, userId);
  },
...
class SupabaseMigration {
  ...
  async migrateBooks() {
    ...
    const books = await this.oldDb.query('SELECT * FROM books');
    
    for (const book of books.rows) {
      try {
        const newUserId = userMapping[book.user_id];
        if (!newUserId) {
          console.warn(`No user mapping found for book ${book.id}`);
          continue;
        }
  ...
  async getUserMapping() {
    ...
    for (const oldUser of oldUsers.rows) {
      try {
        const { data } = await this.supabase.auth.admin.getUserByEmail(oldUser.email);
        if (data?.user) {
  ...
  async verifyMigration() {
    ...
    const oldBookCount = await this.oldDb.query('SELECT COUNT(*) FROM books');
    ...
    console.log(`Users: ${oldUserCount.rows[0].count} → ${newUserCount}`);
    console.log(`Books: ${oldBookCount.rows[0].count} → ${newBookCount}`);
...
- [ ] Enhanced book routes with search
...
**This implementation guide provides the complete technical foundation for Book Catalog V2 development!** 🚀
